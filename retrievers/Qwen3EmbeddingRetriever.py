...
def print_cuda_memory(tag=""):
    if torch.cuda.is_available():
        ...
        print(f"{tag}  GPU Free: {free / 1024 ** 3:.2f} GB | Total: {total / 1024 ** 3:.2f} GB")
...
class Qwen3EmbeddingRetriever:
    def __init__(
        ...
        self.model = SentenceTransformer(self.model_name, model_kwargs={"torch_dtype": "auto"},)
        self.tokenizer = self.model.tokenizer
        test_emb = self.model.encode(
            [self.corpus[0]], batch_size=1, max_length=self.max_length, normalize_embeddings=True
        )
        ...
        if index_path and os.path.exists(index_path):
            ...
            if self.emb_path and os.path.exists(self.emb_path):
                self.embeddings = np.memmap(self.emb_path, dtype=np.float32, mode="r", shape=(n_docs, emb_dim))
            ...
            # Prepare memmap
            self.embeddings = np.memmap(self.emb_path, dtype=np.float32, mode="w+", shape=(n_docs, emb_dim))
            ...
            for start in range(0, n_docs, self.chunk_size):
                ...
                print(f"Chunk {start}-{end}: max char len = {max_char}, max token len = {max_tok}")
                ...
                if self.debug:
                    ...
                    print(f"chunk_embs.shape: {chunk_embs.shape}, dtype: {chunk_embs.dtype}")
